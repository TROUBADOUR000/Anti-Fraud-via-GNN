...
class ModelHandler(object):
	...
	def __init__(self, config):
		...
		if args.data_name == 'yelp':
			...
			idx_train, idx_rest, y_train, y_rest = train_test_split(
				valid_indices, labels[valid_indices], stratify=labels[valid_indices], test_size=args.train_ratio,
				random_state=2, shuffle=True
			)
			idx_valid, idx_test, y_valid, y_test = train_test_split(idx_rest, y_rest, stratify=y_rest,
			...
			self.__x_t__ = non_zero_indices.tolist()
			self.__y_t__ = labels[non_zero_indices].tolist()

		idx_train = idx_train.tolist()
		idx_test = idx_test.tolist()
		idx_valid = idx_valid.tolist()

		print(f'Run on {args.data_name}, postive/total num: {np.sum(labels[labels >= 0])}/{len(labels)}, train num {len(y_train)},'+
	...
	def train(self):
		...
		for epoch in range(args.num_epochs):
			...
			if epoch and epoch % args.valid_epochs == 0:
				if args.model == 'SAGE' or args.model == 'GCN':
		...
		# 分割数据为小批量
		for i in range(0, len(self.__x_t__), batch_size):
			batch_x_t = self.__x_t__[i:i + batch_size]
			batch_y_t = self.__y_t__[i:i + batch_size]
		...
		# 创建结果的 DataFrame
		result_df = pd.DataFrame({'index': self.__x_t__, 'predict': label_list1})

		result_df.to_csv('PC_GNN.csv', index=False)
		print('finished')

		f1_mac_test, f1_1_test, f1_0_test, auc_test, gmean_test = 0, 0, 0, 0, 0
