...
def load_data(data):
	...
	if data == 'yelp':
		...
		data_file = loadmat(prefix + 'fraud.mat')
		labels = data_file['label'].flatten()
		feat_data = data_file['features'].todense().A
		# load the preprocessed adj_lists
		with open(prefix + 'fraud_homo_adjlists.pickle', 'rb') as file:
			homo = pickle.load(file)
		file.close()
		with open(prefix + 'fraud_rur_adjlists.pickle', 'rb') as file:
			relation1 = pickle.load(file)
		file.close()
		with open(prefix + 'fraud_rtr_adjlists.pickle', 'rb') as file:
			relation2 = pickle.load(file)
		file.close()
		with open(prefix + 'fraud_rsr_adjlists.pickle', 'rb') as file:
			relation3 = pickle.load(file)
		file.close()
