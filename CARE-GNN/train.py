...
parser.add_argument('--data', type=str, default='fraud', help='The dataset name. [yelp, amazon]')
...
parser.add_argument('--num-epochs', type=int, default=51, help='Number of epochs.')
...
if args.data == 'yelp':
	...
	idx_train, idx_test, y_train, y_test = train_test_split(
		valid_indices, labels[valid_indices], stratify=labels[valid_indices], test_size=0.95, random_state=2,
		shuffle=True
	)
	...
	non_zero_indices = np.where((labels != 0) & (labels != 1))[0]

	x_t = non_zero_indices.tolist()
	y_t = labels[non_zero_indices].tolist()
...
for epoch in range(args.num_epochs):
	...
	for batch in range(num_batches):
		...
		optimizer.zero_grad()
		if args.cuda:
			loss = gnn_model.loss(batch_nodes, Variable(torch.tensor(batch_label, dtype=torch.long, device='cuda')))
		else:
	...
	total_loss.append(float(loss.item() / num_batches))

	# testing the model for every $test_epoch$ epoch
	if epoch and epoch >= 50 and epoch % args.test_epochs == 0:
		if args.model == 'SAGE':
...
plt.plot(range(args.num_epochs), total_loss, label='Loss')
...
for i in range(0, len(x_t), batch_size):
	batch_x_t = x_t[i:i+batch_size]
	batch_y_t = y_t[i:i+batch_size]

	# 进行预测
	gnn_prob, label_prob1 = gnn_model.to_prob(batch_x_t, batch_y_t, train_flag=False)

	label_list1.extend(label_prob1.data.cpu().numpy()[:, 1].tolist())

label_list1_abs = np.abs(label_list1)

# 创建结果的 DataFrame
result_df = pd.DataFrame({'index': x_t, 'predict': label_list1})

result_df.to_csv('CARE_GNN.csv', index=False)
print('finished')